...
int _connect()
{
	...
	port = atoi(_config._server_param->_report_server.xreport_port);

	//printf("_config.xreport_server = %s, port = %d\n", _config.xreport_server, port);
	if(_config._server_param->_report_server.xreport_server == NULL || port <= 0)
	{
		dzlog_info("upload server is error");
		return -1;
	}

	sdk_resolve(_config._server_param->_report_server.xreport_server, port, &sock);
...
TODO:
	...
	if (ret < 0)
	{
		...
		{
			dzlog_info("connect %s:%d error!\n", _config._server_param->_report_server.xreport_server, port);
			return -1;
		}
	}
...
RERECV:
	...
	if (val > 0)
	{
		...
		{
			dzlog_debug("recv ok buf = %s",buf);
			close(m_sock);
			return 0;
		}
		else 
		{
			dzlog_debug("recv failed buf = %s\n", buf);
			close(m_sock);
			return 1;
		}
